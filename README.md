...
This repository contains a static frontend in `frontend/` and an API backend that runs separately.

The Netlify configuration is committed and ready:

- `netlify.toml`: publishes the `frontend` directory and runs no build step
...
- Domain safety check: `EXPECTED_PGROK_DOMAIN=starpark.app`

Before enabling the tunnel service, verify `~/.pgrok/config` points to your StarPark tunnel server and domain.
