 ...
 * Uses anymap-ts (via AnymapBridge) for map rendering
 * @version 2.0.0
 */
...
class LinkSpotApp {
    ...
    constructor(options = {}) {
        ...
        // State - map is now an AnymapBridge instance
        this.state = {
            map: null,
    ...
    async init() {
        ...
        // Initialize map via anymap-ts bridge
        await this.initMap();
    ...
    /**
     * Initialize map using anymap-ts LeafletRenderer via AnymapBridge
     * @private
     */
    async initMap() {
        const { lat, lon, zoom } = this.config.initialPosition;

        // Create map using AnymapBridge (wraps anymap-ts LeafletRenderer)
        const bridge = new AnymapModule.AnymapBridge('map', { lat, lon, zoom });
        await bridge.init();

        // Disable default zoom control since bridge adds it via Leaflet;
        // add our own at bottom-right
        bridge.addZoomControl('bottomright');

        this.state.map = bridge;

        // Store initial position
        this.state.currentPosition = { lat, lon };

        // Listen for map movement via bridge events
        this.state.map.on('moveend', (center) => {
            this.state.currentPosition = { lat: center.lat, lon: center.lon };
        });
        ...
        this.state.map.on('moveend', (center) => {
            ...
            moveTimeout = setTimeout(() => {
                this.loadHeatMap(center.lat, center.lon);
            }, 500);
        });
    }
    ...
    /**
     * Render grid cells on the map using anymap-ts bridge
     * @param {Object} geojsonData - GeoJSON FeatureCollection of grid cells
     */
    renderGridCells(geojsonData) {
        const self = this;

        // Use bridge to add GeoJSON layer with custom styling and events
        this.state.map.addGeoJSON(
            'grid',
            geojsonData,
            (feature) => {
            ...
            (feature, layer) => {
                ...
                layer.on('click', () => {
                    ...
                    self.showPointDetails(center.lat, center.lon, feature.properties);
                });
    ...
    /**
     * Render building footprints on the map using anymap-ts bridge
     * @param {Object} geojsonData - GeoJSON FeatureCollection of buildings
     */
    renderBuildings(geojsonData) {
        if (!geojsonData || !geojsonData.features) return;

        // Use bridge to add building GeoJSON layer
        this.state.map.addGeoJSON(
            'buildings',
            geojsonData,
            {
        ...
        // Ensure buildings render below grid cells
        if (this.state.map.hasLayer('grid')) {
            this.state.map.bringToBack('buildings');
        }
    }
    ...
    async centerOnGPS() {
        ...
        try {
            ...
            this.state.map.addMarker('gps-location', latitude, longitude, {
                popup: 'Your Location',
                openPopup: true
            });
