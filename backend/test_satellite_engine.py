...
SAMPLE_TLE_DELTA_DATA = """STARLINK-1008
1 44714U 19074B   24359.00000000  .00020000  00000-0  12345-3 0  9998
2 44714  53.0000 121.0000 0001000  90.0000 270.0000 15.50010000 12349
STARLINK-2001
1 54717U 23001A   24359.00000000  .00010000  00000-0  12345-3 0  9991
2 54717  53.0000 300.0000 0001000  90.0000 270.0000 15.50000000 12350
"""
...
class TestSatelliteEngine(unittest.TestCase):
    ...
    def test_merge_tle_sets_replaces_and_appends(self):
        """Test merging incremental Space-Track GP updates into baseline set."""
        if not self.engine_available:
            self.skipTest("Engine not available")

        engine = self.SatelliteEngine(self.mock_redis)
        merged = engine._merge_tle_sets(SAMPLE_TLE_DATA, SAMPLE_TLE_DELTA_DATA)
        merged_lines = merged.splitlines()

        self.assertIn("STARLINK-2001", merged)
        self.assertIn(
            "1 44714U 19074B   24359.00000000  .00020000  00000-0  12345-3 0  9998",
            merged,
        )
        self.assertEqual(merged_lines.count("STARLINK-1008"), 1)

    @patch("satellite_engine.requests.Session")
    def test_refresh_space_track_uses_delta_query_when_seed_exists(
        self, mock_session_cls
    ):
        """Test Space-Track refresh uses CREATION_DATE delta query when seeded."""
        if not self.engine_available:
            self.skipTest("Engine not available")

        from satellite_engine import SPACE_TRACK_STARLINK_GP_DELTA_URL
        ...
        data_response = Mock()
        data_response.text = SAMPLE_TLE_DELTA_DATA
        data_response.raise_for_status = Mock()
        session.get.return_value = data_response

        engine = self.SatelliteEngine(
            self.mock_redis,
            space_track_identity="user@example.com",
            space_track_password="top-secret",
        )

        merged = engine._refresh_space_track_tles(current_tle_data=SAMPLE_TLE_DATA)

        session.get.assert_called_once_with(
            SPACE_TRACK_STARLINK_GP_DELTA_URL,
            timeout=engine.space_track_timeout_seconds,
        )
        self.assertIn("STARLINK-2001", merged)
        self.assertIn(
            "1 44714U 19074B   24359.00000000  .00020000  00000-0  12345-3 0  9998",
...
class TestAzimuthElevationAccuracy(unittest.TestCase):
    ...
    def test_elevation_accuracy_tolerance(self):
        ...
        for sat in engine._satellites:
            ...
            self.assertLess(
                az_diff, 0.5, f"Azimuth difference {az_diff}° exceeds 0.5° tolerance"
            )
...
class TestPerformance(unittest.TestCase):
    ...
    def test_full_constellation_performance(self):
        ...
        positions = engine.get_satellite_positions(
            37.7749, -122.4194, timestamp=test_time
        )
        ...
        self.assertLess(
            elapsed_ms, 500, f"Computation took {elapsed_ms:.2f}ms, expected < 500ms"
        )
    ...
    def test_convenience_function(self):
        ...
        with patch("satellite_engine.requests.get") as mock_get:
            ...
            result = self.get_visible_starlink_satellites(
                lat=37.7749, lon=-122.4194, min_elevation=0.0
            )
